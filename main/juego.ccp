...
{ //Ve si puede hacer mov, si es asi mueve
    ...
    case 0:
    { // ARRIBA
        if (puede_hacer_mov(juego, i) && juego.mina.plano[juego.mina.x + incF[i]][juego.mina.y + incC[i]] != PIEDRA)
        {                                       // Puede hacer mov y la casilla no es piedra                                                                                                                    //Puede hacer mov
    ...
    { // ABAJO
        i = 1;
        if (puede_hacer_mov(juego, i) && juego.mina.plano[juego.mina.x + incF[i]][juego.mina.y + incC[i]] != PIEDRA)
        {                                       //Puede hacer mov y la casilla no es piedra
            estado = casilla_deplaza(juego, i); //Valoramos casos especiales que haya una gema, salida
    ...
    case 6:
    { //TNT
        if (juego.mina.plano[juego.mina.x + incF[1]][juego.mina.y + incC[1]] == LIBRE)
        { // Para poder colocar el TNT
...
{ // valoramos casos especiales sobre la casilla a la que se desplaza
    tEstado estado = JUGANDO;
    if (juego.mina.plano[juego.mina.x + incF[i]][juego.mina.y + incC[i]] == SALIDA)
    { // Llega a la salida
        estado = FIN;
    }
    if (juego.mina.plano[juego.mina.x + incF[i]][juego.mina.y + incC[i]] == GEMA)
    { //Llega a una gema
        juego.gem++;
    }
    if (juego.mina.plano[juego.mina.x + incF[i]][juego.mina.y + incC[i]] == PIEDRA && movilidad_piedra)
    { // Hay una piedra movible
...
{ //Vemos si la piedra puede ser desplazada o no
    bool movilidad = true;
...
{// derrumbamiento en la mina
    ...
    { //Mientras no tengamos un muro o el minero
        x = iniX;
        y = iniY;
        while (juego.mina.plano[x + incF[1]][y + incC[1]] == LIBRE)
...
{ // Tenemos la posibilidad de que el minero este soportando una piedra y al desplazarse esta caiga
    if (juego.mina.plano[juego.mina.x + incF[0]][juego.mina.y + incC[0]] == PIEDRA)
    {
        mover(juego, i);
...
{ //Explosion TNT
    ...
    for (int i = 0; i <= 7; i++)
    {
        if (juego.mina.plano[coordX + incF[i]][coordY + incC[i]] == MINERO)
        { // Si alguna de las casilla donde alcanza la dinamita es el minero game over
            estado = OVER;
        }
